...
SpacedPaths = namedtuple("SpacedPaths", ["path_without_spaces", "path_with_spaces", "path_with_single_quote"])
...
class MyReporter(TerminalReporter):
    def short_test_summary(self):
        ...
        for rep in failed:
            # breakpoint()
            self.write_line(f"[FAILED] {rep.head_line} - {rep.longreprtext.splitlines()[-3]}")

        errored = self.stats.get("error", [])
        for rep in errored:
            # breakpoint()
            self.write_line(f"[ERROR] {rep.head_line} - {rep.longreprtext.splitlines()[-3]}")
...
def pytest_addoption(parser):
    parser.addoption("--corba", action="store_true", default=False, help="run CORBA tests")
    parser.addoption("--console", action="store_true", default=False, help="run console tests")
    parser.addoption("--gui", action="store_true", default=False, help="run GUI tests")
    parser.addoption("--only-gui", action="store_true", default=False, help="run only GUI tests")
...
def mapdl_console(request):
    if os.name != "posix":
        raise RuntimeError('"--console" testing option unavailable.  ' "Only Linux is supported.")
...
def coupled_example(mapdl, cleared):
    ...
    mapdl_code = mapdl_code.replace("SOLVE", "SOLVE\n/COM Ending script after first simulation\n/EOF")
